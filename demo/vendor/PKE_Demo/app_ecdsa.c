...
unsigned int ecdsa_std_sign_verify_test_internal(eccp_curve_t *curve, char *curve_name, unsigned char *E, unsigned int EByteLen,
		...
		unsigned char sign_ret_value, char *info)
{
    ...
    if(ECDSA_SUCCESS == ret)
    {
    	...
    	{
			if(memcmp(buf, signature, signatureByteLen))
			{
...
//	gpio_write(LED2, 0);
	...
	for(i=0;i<ECDSA_SIGN_VERIFY_ROUND1;i++)
	{
		...
		if(ECDSA_SUCCESS != ret)
		{
			printf("\r\n ECDSA %d get key error, ret = %02x", curve->eccp_p_bitLen, ret);
			return 1;
		}

		for(j=0;j<50;j++)
		{
			printf("\r\n ECDSA %d round1=%d, round2=%d", curve->eccp_p_bitLen, i+1, j+1);

			ret = ecdsa_sign(curve, E, EByteLen, k, priKey, signature);
			if(ECDSA_SUCCESS != ret)
			{
				printf("\r\n ECDSA %d sign error, ret = %02x", curve->eccp_p_bitLen, ret);
				return 1;
			}
			...
			if(ECDSA_SUCCESS != ret)
			{
				printf("\r\n ECDSA %d verify error, ret = %02x", curve->eccp_p_bitLen, ret);
				return 1;
			}
		}
	}
...
/**
 * @brief		ecdsa speed test.
 * @return		ECDSA_SUCCESS(success); other(error).
 */
unsigned int ecdsa_speed_test(eccp_curve_t *curve)
{
	unsigned char priKey[66];
	unsigned char pubKey[132];
	unsigned char E[66];
	unsigned char signature[132];
	unsigned int i, EByteLen;
	unsigned int ret;
