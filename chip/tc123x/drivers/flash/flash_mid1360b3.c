...
unsigned char flash_write_status_mid1360b3(unsigned short data, unsigned int mask)
{
	...
	unsigned short status = flash_read_status_mid1360b3();
	if(data != (status & mask))	//To reduce the operation of the status register.
	{
		...
		status = flash_read_status_mid1360b3();
	}
	if(data == (status & mask))
	{
		return 1;
...
{
	return flash_write_status_mid1360b3(data, FLASH_WRITE_STATUS_BP_MID1360B3);
}
...
{
	return flash_write_status_mid1360b3(FLASH_LOCK_NONE_MID1360B3, FLASH_WRITE_STATUS_BP_MID1360B3);
}
...
{
	return flash_read_status_mid1360b3()&FLASH_WRITE_STATUS_BP_MID1360B3;
}
...
{
	return flash_write_status_mid1360b3(data, FLASH_WRITE_STATUS_OTP_MID1360B3);
}
