 ...
 * 
 */
unsigned char uart_TxIndex[1] = {0};
...
void uart_ndma_send_byte(uart_num_e uart_num,unsigned char uartData)
{
    ...
    reg_uart_data_buf(uart_TxIndex[uart_num]) = uartData;
 ...
 * 
 */
unsigned char uart_RxIndex[1] = {0};
...
{
    unsigned char rx_data = reg_uart_data_buf(uart_RxIndex[uart_num]);
...
void uart_set_rts(uart_num_e uart_num,unsigned char Enable, UART_RTSModeTypeDef Mode, unsigned char Thresh, unsigned char Invert,  UART_RtsPinDef pin)
{
    if (Enable)
    {
        if (pin == UART_RTS_PA5)
        {
            gpio_set_func(pin, 3); //enable rts pin
        }
        else if (pin == UART_RTS_PA7)
        {
            gpio_set_func(pin, 0); //enable rts pin
        }
...
void uart_set_cts(uart_num_e uart_num,unsigned char Enable, unsigned char Select, UART_CtsPinDef pin)
{
    if (Enable)
    {
        ...
        if (pin == UART_CTS_PA4)
        {
            gpio_set_func(pin, 0);//enable cts pin
        }
        else if(pin == UART_CTS_PA6)
        {
            gpio_set_func(pin, 2);//enable cts pin
        }
...
void uart_gpio_set(uart_num_e uart_num, UART_TxPinDef tx_pin, UART_RxPinDef rx_pin)
{
    ...
    {
        if (tx_pin == UART_TX_PA6)
            uart_set_pin_mux(tx_pin, 0);
        else
            uart_set_pin_mux(tx_pin, UART_TX);
    }
    if (rx_pin != UART_RX_NONE_PIN)
    {
        if(rx_pin == UART_RX_PA5)
            uart_set_pin_mux(rx_pin, 2);
        else
