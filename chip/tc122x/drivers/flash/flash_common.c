 ...
 *  If you add a new flash, you need to check the following location and add the corresponding flash model:
	...
	#7 If you add new flash (especially ZB), you need to confirm the new model, whether there is a problem that the lower the flash voltage, the longer the flash write time (BLE need to write 16bytes time is less than 200us),
	   if there is a problem, you need to modify the user_read_flash_value_calib function.(added by xiaobin.huang 20250802)
    #8 When adding new flash, you need to confirm the maximum operating current, not greater than 10mA. the larger the load on the LDO of the flash of the TC122X, the lower the VDD_F voltage will be,
       the present voltage settings are in accordance with the maximum operating current of the flash is evaluated at 10mA, if the maximum operating current is greater than 10mA, you need to re-assessment.(added by xiaobin.huang 20250802)
