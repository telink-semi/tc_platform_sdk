 ...
 *  2:Just has 0.6V/0.9V/1.2V reference voltage.
 *  3:Resolution:8/10/12/14.
 *  4:input mode,just has differential
 *  5:ADC_INTER_TEST is for internal testing only.
 */
...
/**
* 		The B89 ADC deletes the original Vbat div configuration register, adds the Vbat div enable register, and only has 1/3 voltage division.
* 		Enabling this BIT increases the operating current.
* 		The cpu_sleep_wakeup function will do the corresponding processing to ensure that the current is normal.
*		changed by jiarong.20220306. reported by haitao.
**/
#define areg_adc_vref_vbat_div			0x01
enum{
	FLD_ADC_VREF_VBAT_DIV   = 	        BIT(7),
};
/**
...
{
	analog_write(areg_adc_vref_vbat_div, (analog_read(areg_adc_vref_vbat_div)&(~FLD_ADC_VREF_VBAT_DIV)) | (vbat_div<<7));
	if(vbat_div)
	{
		adc_vbat_divider=3;
	}
