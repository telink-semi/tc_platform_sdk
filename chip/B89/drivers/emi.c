...
/**
 * @brief   This function serves to set singletone power.The power setting is related to the power supply mode.
 * 			When switching between the two power supply modes through pm_set_1p2_power_sel, this function needs
 * 			to be called again to set the energy.The energy in Vbat power supply mode is based on 1.5V voltage
 *			as the reference standard.
 * @param   level - the power level index.
 * @return  none.
 */
 ...
 * @param   power_level - the power level index.
...
void rf_emi_tx_burst_loop_ramp(RF_ModeTypeDef rf_mode,unsigned char pkt_type)
{
	...
	if(1 == pm_1p2_power_sel_vbat)
	{
		power = (unsigned char)rf_power_Level_list[read_reg8(0x40008)];
	}
	else
	{
		power = (unsigned char)rf_power_Level_list[read_reg8(0x40008) + 19];
	}
