...
{
	unsigned char val_sda=0;
	unsigned char val_scl=0;
	if((sda_pin == I2C_GPIO_SDA_A0)||(sda_pin == I2C_GPIO_SDA_A2)||(sda_pin == I2C_GPIO_SDA_A5)||(sda_pin == I2C_GPIO_SDA_B3))
	{
		val_sda=BIT(0);
	}

	else if((sda_pin == I2C_GPIO_SDA_B5) || (sda_pin == I2C_GPIO_SDA_D7))
	{
		val_sda=BIT(1);
	}

	if((scl_pin == I2C_GPIO_SCL_A1) || (scl_pin == I2C_GPIO_SCL_B0))
	{
		val_scl=BIT(0);
	}
	else if(scl_pin == I2C_GPIO_SCL_A4)
	{
		val_scl=(BIT(0)|BIT(1));
	}
	else if(scl_pin == I2C_GPIO_SCL_B3)
	{
		val_scl=BIT(2);
	}
	else if((scl_pin == I2C_GPIO_SCL_B7) || (scl_pin == I2C_GPIO_SCL_D6))
	{
		val_scl=BIT(1);
	}
	...
	gpio_set_func(scl_pin, val_scl);
	gpio_set_func(sda_pin, val_sda);
