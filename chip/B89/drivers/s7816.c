...
{
	if(pin_7816_clk==S7817_CLK_PB2)
	{
	    gpio_set_func(pin_7816_clk,PB2_UART_7816_CLK);
	}
	if(pin_7816_clk==S7817_CLK_PB6)
	{
		gpio_set_func(pin_7816_clk,PB6_UART_7816_CLK);
	}
}
...
void s7816_set_trx_pin(S7816_TRx_PinDef Pin_7816_TRX)
{
	...
	if(Pin_7816_TRX==S7816_TRX_PB1)
	{
		val_trx=BIT(0);
	}
	else if(Pin_7816_TRX==S7816_TRX_PD3)
	{
		val_trx=BIT(1);
	}
	gpio_set_func(Pin_7816_TRX,val_trx); // set rx pin
}
