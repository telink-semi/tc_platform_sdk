...
void uart_set_rts(unsigned char Enable, UART_RTSModeTypeDef Mode, unsigned char Thresh, unsigned char Invert,  UART_RtsPinDef pin)
{
    if (Enable)
    {
    	unsigned char val_rts=0;
    	if(pin==UART_RTS_PD2)
    	{
    	      val_rts=BIT(0);
    	}
    	else if(pin==UART_RTS_PA2||pin==UART_RTS_PC2||pin==UART_RTS_PD5)
    	{
    	      val_rts=BIT(1);
    	}
    	else if(pin==UART_RTS_PB5||pin==UART_RTS_PC0||pin==UART_RTS_PD3)
    	{
    	      val_rts=BIT(1)|BIT(0);
    	}
    	else if(pin==UART_RTS_PB4)
    	{
    	      val_rts=BIT(2)|BIT(0);
    	}
    	gpio_set_func(pin,val_rts);//enable rts pin
...
void uart_set_cts(unsigned char Enable, unsigned char Select,UART_CtsPinDef pin)
{
    if (Enable)
    {
    	unsigned char val_cts=0;
        if(pin==UART_CTS_PC2)
        {
        	val_cts=BIT(0);
        }
        else if(pin==UART_CTS_PA3||pin==UART_CTS_PD2)
        {
        	val_cts=BIT(1);
        }
        else if(pin==UART_CTS_PB4||pin==UART_CTS_PB6)
        {
        	val_cts=BIT(1)|BIT(0);
        }
        else if(pin==UART_CTS_PC0)
        {
        	val_cts=BIT(2);
        }
        else if(pin==UART_CTS_PB3)
        {
        	val_cts=BIT(2)|BIT(0);
        }
    	...
    	gpio_set_func(pin,val_cts);//enable cts pin
...
void uart_gpio_set(UART_TxPinDef tx_pin,UART_RxPinDef rx_pin)
{
	...
	if(tx_pin != UART_TX_NONE_PIN){
		...
		else if(tx_pin==UART_TX_PA1||tx_pin==UART_TX_PB0||tx_pin==UART_TX_PB6||tx_pin==UART_TX_PC0||tx_pin==UART_TX_PD4)
		{
			val_tx=BIT(1);
		}
		else if(tx_pin==UART_TX_PB3)
		{
			val_tx=BIT(0)|BIT(1);
		}
		...
		gpio_set_func(tx_pin,val_tx); // set tx pin
	}
	if(rx_pin != UART_RX_NONE_PIN){
		if(rx_pin==UART_RX_PB1||rx_pin==UART_RX_PC0)
		{
			val_rx=BIT(0);
		}
		else if(rx_pin==UART_RX_PA0||rx_pin==UART_RX_PA5||rx_pin==UART_RX_PB3||rx_pin==UART_RX_PD3)
		{
			val_rx=BIT(1);
		}
		else if(rx_pin==UART_RX_PC2)
		{
			val_rx=BIT(1)|BIT(0);
		}
		else if(rx_pin==UART_RX_PB0||rx_pin==UART_RX_PB4)
		{
			val_rx=BIT(2);
		}
		...
		gpio_set_func(rx_pin,val_rx); // set rx pin
	}
}
...
void uart_set_rtx_pin(UART_RTxPinDef rtx_pin)
{
	...
	gpio_setup_up_down_resistor(rtx_pin, PM_PIN_PULLUP_10K);
 	if(rtx_pin==UART_RTX_PB1)
 	{
 		gpio_set_func(rtx_pin,PB1_UART_RX);
 	}
 	else if(rtx_pin==UART_RTX_PD3)
 	{
 		gpio_set_func(rtx_pin,PD3_UART_RX);
 	}
}
